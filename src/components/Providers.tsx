...
import { PRODUCTS, FREE_EXPRESS_FROM, bySlug } from '@/lib/catalog';
...
type Shop = {
  cart: CartLine[];
  add: (id: string, q?: number, options?: { openCart?: boolean; allowQuote?: boolean }) => void;
  remove: (id: string) => void;
  count: number;
  subtotal: number;
  missingForFreeExpress: number;
  favs: string[];
...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  const add = useCallback((id: string, q = 1, options: { openCart?: boolean; allowQuote?: boolean } = {}) => {
    const p = bySlug(id);
    if (!p || (!p.priceKnown && !options.allowQuote)) return;
    setCart((c) => {
      const hit = c.find((l) => l.id === id);
      return hit ? c.map((l) => (l.id === id ? { ...l, q: l.q + q } : l)) : [...c, { id, q }];
    });
  ...
  const subtotal = useMemo(
    () => cart.reduce((s, l) => s + (bySlug(l.id)?.price ?? 0) * l.q, 0),
    [cart],
  );
  ...
  const value: Shop = {
    ...
    subtotal,
    missingForFreeExpress: Math.max(0, FREE_EXPRESS_FROM - subtotal),
